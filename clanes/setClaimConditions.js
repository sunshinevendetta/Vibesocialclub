...
async function main() {
  ...
  // Remove trailing slash from RPC_URL if present
  const rpcUrl = process.env.RPC_URL.replace(/\/$/, "");

  // Initialize the SDK with your private key and the modified RPC URL
  const customRpcProvider = new ethers.providers.JsonRpcProvider(rpcUrl);
